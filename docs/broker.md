...
## 多会话模式（`multiChat`，direct 档）

想让**一个** pi 会话同时接私聊和群 @，用 `multiChat: true`（不需要 broker）：
...
- 不再做会话级绑定过滤 —— 私聊、群里 @ 的消息都进同一个 pi 会话
- 每个回合的输出**回到触发它的那个对话**：流式卡片、补发的全文、审批卡片都是
- 群里别人的话不会进上下文 —— `requireMention: true` 时飞书侧的策略管道就把没 @
  机器人的群消息拒掉了，根本到不了扩展这层。**群场景下这个值必须是 `true`**

### 来源归属在哪些情况下会错

pi 的 `agent_start` 事件**不带「是哪条消息触发的」**，所以来源只能靠顺序推断：
转发消息给 pi 时入队，回合开始时出队。常规路径（含 `followUp` 排队）顺序是对的，
但有两处对不上，用之前要知道：

1. **终端自己敲的回合没有来源** —— 出队拿到空，退回网关的默认收件方（已绑定会话）。
   不会乱发给上一个来源
2. **回合中途被另一个对话 `steer` 打断** —— 输出仍然回到最初那个来源。被打断者看到
   自己的话影响了别处的回合

真要严格隔离（不同项目、不同人各自独立上下文），用 broker 模式一对话一会话，
而不是 `multiChat`。`multiChat` 适合的是「同一个人从两个入口找同一个 agent」。

`multiChat` 与 broker 档**不叠加** —— broker 下一个会话本就只绑一个对话，
收件方由 broker 的路由表决定。
