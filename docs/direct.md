...
但 `pi.appendEntry(customType, data)` 能往会话里写自定义条目，`ctx.sessionManager.getEntries()`
能把它们按**实际顺序**读回来。所以扩展在转发消息前写一条 `pi-feishu-origin` 条目记下
chatId，`agent_start` 时倒着找最后一条用户消息、再看它前面紧邻的来源条目（遇到另一条
用户消息就停，那属于上一轮）。

那份顺序是 pi 自己处理消息的 ground truth，比在旁边维护队列或按文本匹配都可靠：

- 两个对话发了一模一样的文本也不会配错（按位置，不按文本）
- 不需要「一个回合恰好一个槽位」这种站不住的前提 —— steer / followUp 是否被合并进
  当前回合都不影响
- 终端敲的消息前面没有来源条目，天然区分得出来，走默认收件方

`appendEntry` 写的普通自定义条目**不进 LLM 上下文**（pi 的 `sessionEntryToContextMessages`：
plain custom entries do not participate in context），所以 chatId 不会污染 agent 看到的
对话 —— 这也是不采用「把标记编进消息文本」那个方案的理由。
