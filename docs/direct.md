...
pi 把排队消息（`followUp` / `steer`）并进**同一个** agent 运行：只有一次 `agent_start`、
一次 `agent_end`。所以「私聊正在跑，群里 @ 一句」如果直接投给 pi，答案会整段发进
**私聊**，群里只剩一个表情、一个字都收不到。
...
`agent_start`，输出自然回到群里。**同一个对话**发来的消息不受影响，照旧并进当前回合 ——
反正当前回合的流就发往那儿。
