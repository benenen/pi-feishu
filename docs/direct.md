...
来源队列的对齐规则是「**一个回合恰好对应一个槽位**」：

- 飞书消息入队一个来源；`!` 开头的 **steer 不入队**（它插进当前回合，不开新回合）
- **终端在空闲时敲的输入会入队一个空槽位** —— 不占位的话，它会认领掉排在前面的
  飞书来源，把终端的输出发进那个对话，而飞书那条消息的回合回落到已绑定会话

仍有一处对不上：**流式进行中在终端敲的输入**被 pi 排成 followUp 时会晚一步开回合，
而此刻分不清它是 steer 还是 followUp，所以不入队 —— 那个回合会认领掉一个飞书来源。
回合中途被另一个对话 steer 打断时，输出也仍回最初那个来源。
