...
- 取值是**飞书的具名表情 key**，不是 Unicode 表情。`"EYES"` 是默认值
- 置**空字符串**关闭这个行为
- 加不上表情**不影响消息处理** —— 内部自兜异常，只在日志留一条 warning。
  所以如果你的租户不认 `EYES` 这个 key，表现是「没有表情，但消息照常处理」，
  换成别的 key（比如 `"OK"`）即可

> 这个 key 的有效性没有在真实飞书环境验证过（开发沙箱不出网）。第一次用如果没看到
> 表情，去看日志里有没有「加表情回应失败」，有的话就是 key 不对。
