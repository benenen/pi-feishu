...
注意这**不是**「不执行任意代码」。`npm test` 跑的是仓库自己的测试脚本 —— 在边界之内，理应放行，否则编码代理没法干活。而 `git log --output=/etc/cron.d/pwn` 越界，必须拦。

### 为什么判定要做到标志一层

前两版都栽在同一类错误上，只是层次不同：

- **v1 黑名单**（枚举危险命令）：`echo hi>/etc/x` 少个空格就漏、`dd of=… if=…` 换个参数序就漏、`find / -delete` 根本不含关键字。
- **v2 命令白名单**（枚举安全命令，不看标志）：实测 `git log -1 --format=format:"* * * * * root touch /tmp/pwned" --output=./PWNED.txt` **确实把任意内容写进了任意路径**，且全程无 shell 元字符。`sort -o FILE`、`find -fprint0 FILE` 同样可写。

根因：几乎每个「只读命令」都带着能写文件的标志。所以标志也必须白名单化 —— 出现未列出的 `-xxx` 就弹审批。

`bash` 判定顺序：

1. 含任一 shell 元字符（`| & ; < > $ ` ( ) { }`、换行、反斜杠）→ **risky**。
2. 首 token（小写后）在「子命令表」里：第二个 token 必须属于该表的只读子命令集，且**其余标志全部通过 `flagsAllowed`**，否则 risky。
3. 首 token 在「命令表」里：**标志全部通过 `flagsAllowed`**，否则 risky。
4. 其余一律 **risky**。

标志检查刻意**保留大小写**（只有命令名和子命令查表时才小写）：`-A` 与 `-a` 在多数命令里语义不同，混为一谈会让未授权的标志混进来。

已从表里剔除的条目及理由：`sort`/`tree`（`-o` 写文件）、`hostname`（可设置主机名）、`printenv`/`env`（向聊天记录泄露密钥）、`git branch`/`tag`/`remote`（`-D` 删分支、`-d` 删标签、`set-url` 改远端）、`npm run`（执行任意脚本）、`go build`（`-o` 写任意路径）、`man`（会起分页器）。
...
Expected: FAIL —— 现有实现没有 `flagsAllowed`，且多条写标志用例返回 `safe`

- [ ] **Step 3: 实现 `extensions/feishu/risk.ts`**
...
Run: `npm test`
Expected: PASS，config 与 risk 全绿

- [ ] **Step 5: 提交**

```bash
git add extensions/feishu/risk.ts test/risk.test.ts
git commit -m "feat: 危险判定下沉到标志层"
```
