...
本任务不写单测：它几乎全是对飞书 SDK 的转接，价值在冒烟测试（Task 8）。逻辑分支（绑定判定）已在 Task 7 的 bridge 契约测试里覆盖。

- [ ] **Step 1: 实现 `extensions/feishu/feishu.ts`**

```ts
import { createLarkChannel } from "@larksuiteoapi/node-sdk";
import type { LarkChannel } from "@larksuiteoapi/node-sdk";
import type { AppendSink } from "./turn-stream.ts";
import type { ApprovalRequest, Asker, Decision } from "./approval.ts";
...
export class FeishuGateway {
  ...
  async connect(): Promise<void> {
    ...
    channel.on("cardAction", (evt) => {
      const value = evt.action.value as { kind?: string; id?: string; allow?: boolean } | undefined;
      if (value?.kind !== "pi-feishu-approval" || typeof value.id !== "string") return;
      const entry = this.#pending.get(value.id);
      if (!entry) return;
      this.#pending.delete(value.id);
      entry.resolve({
        allow: value.allow === true,
        reason: value.allow === true ? "飞书批准" : "飞书拒绝",
      });
      void this.#settleCard(entry.messageId, value.allow === true ? "已批准" : "已拒绝");
    });
  ...
  async disconnect(): Promise<void> {
    ...
    for (const [id, entry] of this.#pending) {
      entry.resolve({ allow: false, reason: "会话已结束" });
  ...
  cardAsker: Asker = async (req, signal) => {
    ...
    const result = await channel.send(to, { card: this.#approvalCard(id, req) });

    return await new Promise<Decision>((resolve) => {
      this.#pending.set(id, { resolve, messageId: result.messageId });
      signal.addEventListener(
        "abort",
        () => {
          if (!this.#pending.delete(id)) return;
          void this.#settleCard(result.messageId, "已在终端处理");
          resolve({ allow: false, reason: "已由其他通道处理" });
        },
        { once: true },
      );
    });
  ...
  #approvalCard(id: string, req: ApprovalRequest): object {
    const detail =
          ...
          actions: [
            {
              ...
              type: "primary",
              value: { kind: "pi-feishu-approval", id, allow: true },
            },
            {
              tag: "button",
              text: { tag: "plain_text", content: "拒绝" },
              type: "danger",
              value: { kind: "pi-feishu-approval", id, allow: false },
            },
          ],
        },
      ],
  ...
  async #settleCard(messageId: string, status: string): Promise<void> {
    try {
      await this.#channel?.updateCard(messageId, {
        config: { wide_screen_mode: true },
...
Run: `npm run typecheck`
Expected: 无输出，退出码 0。若飞书 SDK 的类型名不匹配（例如 `LarkChannel` 未直接导出），按 `node_modules/@larksuiteoapi/node-sdk/types/index.d.ts` 的实际导出调整 import，不要用 `any` 绕过。

- [ ] **Step 3: 确认既有测试未被破坏**

Run: `npm test`
Expected: PASS，前 5 个测试文件仍全绿

- [ ] **Step 4: 提交**

```bash
git add extensions/feishu/feishu.ts
git commit -m "feat: 飞书网关，含卡片审批通道"
```
