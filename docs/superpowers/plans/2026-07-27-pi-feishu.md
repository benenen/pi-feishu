...
`resolvePath` 是注入的符号链接解析器，默认恒等函数（保持纯函数、可单测）。生产环境由 `bridge.ts` 注入 `fs.realpathSync.native` 的容错包装。

**这是整个项目的安全边界。** 它决定哪些工具调用要先经人工批准才能在用户机器上执行，失效是静默的 —— 本该弹审批的命令会直接无人值守跑掉。

判定采取**枚举安全**而非枚举危险。黑名单方案已被实测否决：正则匹配 `rm -rf`/`sudo`/`dd` 等模式时，`echo hi>/etc/x`（少个空格）、`dd of=… if=…`（换参数序）、`chmod 0777`（前导零）、`git -c k=v push --force`（插入全局选项）、`curl -o x.sh …; bash x.sh`（分号拆开）等 8 处零成本绕过全部漏过，且 `find / -delete`、`python3 -c "shutil.rmtree('/')"` 根本不含任何关键字。

`bash` 判定顺序：

1. 含任一 shell 元字符（`| & ; < > $ ` ( ) { }`、换行、反斜杠）→ **risky**。重定向、管道、命令替换、命令串联全由这条拦下。
2. 首 token 是 `find`：带 `-delete`/`-exec`/`-execdir`/`-ok`/`-okdir`/`-fls`/`-fprint`/`-fprintf` → risky，否则 safe。
3. 首 token 在「需看子命令」表里：第二个 token 属于该表的只读集合才 safe，否则 risky。
4. 首 token 在只读命令白名单里 → safe。
5. 其余一律 **risky**。

刻意不解析引号：`grep "foo(bar)" .` 会因括号判为 risky，多弹一次审批。误判方向偏安全，可接受。
...
Expected: FAIL —— `Cannot find module '../extensions/feishu/risk.ts'`
